...
// query db
$sql = "SELECT p.*, s1.name as s1name, s2.name as s2name, s1.dimos as s1dimos, s1.email1 as emails1, s2.email1 as emails2 FROM $prTable p JOIN $schTable s1 ON p.sch1 = s1.id JOIN $schTable s2 ON p.sch2 = s2.id WHERE p.id = $progId";
